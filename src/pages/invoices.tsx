...
const InvoicePage: React.FC = () => {
  ...
  const calculatePackageRate = (rateData: ProductRateForInvoice): number => {
    if (rateData.finalProductRate && rateData.finalProductRate > 0) {
      return Number(rateData.finalProductRate.toFixed(2));
    }

    const finalOilRatePerUnit = Number(rateData.finalOilRatePerUnit || 0);
    const packageSize = Number(rateData.packageSize || 1);
    const packagingAverageCost = Number(rateData.packagingAverageCost || 0);

    return Number(
      (finalOilRatePerUnit * packageSize + packagingAverageCost).toFixed(2)
    );
  };
  ...
  const handleProductChange = async (
    ...
    row.total = Number((rate * qty).toFixed(2));
  ...
  const getPackagingTypesByOilType = (oilType: string): string[] => {
    if (!oilType) return packagingTypes;

    return Array.from(
      new Set(
        finishedGoods
          .filter((item: any) => item.oilType === oilType && item.isActive !== false && Number(item.quantity || 0) > 0)
          .map((item: any) => item.packagingType)
          .filter(Boolean)
      )
    ).sort() as string[];
  };
                ...
                <td style={styles.td}>
                  ...
                  {productRateDetails[index] && (
                    <small style={{ display: "block", marginTop: 4, color: "#555" }}>
                      ₹{productRateDetails[index].finalOilRatePerUnit} ×{" "}
                      {productRateDetails[index].packageSize}
                      {" + "}
